...
def set_velocity(uav_velocity_publisher, vx, vy, vz):
    velocity_msg = TwistStamped()
    velocity_msg.twist.linear.x = vx
    velocity_msg.twist.linear.y = vy
    velocity_msg.twist.linear.z = vz
    uav_velocity_publisher.publish(velocity_msg)


def start_mission(uavs, drone_lat_long, tracker_coordinate, past_target_coordinates, relative_distances, uwb_info):
    uav_tracker = uavs[0]
    stop_distance = 2.0
    stop_number = 0
    count = 0
    ...
    # Get initial positions and start moving the target drone
    initial_move.initial_movement(
        uavs, drone_lat_long, tracker_coordinate, relative_distances, initial_guess, uwb_info)

    while True:
        if relative_distances:  # Check for new distance data
            distance = relative_distances[-1]  # Latest distance
            ...
            if stop_number >= 5:
                break  # Stop if the distance is less than the stop distance

            guess_target_position = gradient_descent_method(
                initial_guess, tracker_coordinate[-10:], relative_distances[-10:])
            velocity = PD_velocity(relative_distances, initial_velocity, 0.5)
            if number > 10:
                target_position = map_estimation(past_target_coordinates[-10:], guess_target_position,
                                                 tracker_coordinate[-5:], relative_distances)
            else:
                target_position = mle_method(
                    guess_target_position, tracker_coordinate[-5:], relative_distances[-5:])

            vx, vy, vz = follow_me(
                tracker_coordinate[-1], target_position, velocity, relative_distances[-1], 0.5)
            uav_tracker.set_velocity(vx, vy, vz, duration=1)
           ...
           # UWB distance
            ...
            rospy.logerr(f"Predit target position = {target_position}")
            past_target_coordinates.append(target_position)
            number += 1

            predict_distance = relative_distances[-1]
...
# 2021/01/04 Maximum Likelihood Estimation
